package com.vivek.encryption.utils;

import java.math.BigInteger;

public class Utils {

    public static void inverseMatrix(int[][] inMatrix, int[][] outMatrix) {
        int row = inMatrix.length;
        int col = inMatrix[0].length;

        BigInteger[][] in = new BigInteger[row][col];
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                in[i][j] = new BigInteger("" + inMatrix[i][j]);
            }
        }

        ModMatrix input = new ModMatrix(in);
        ModMatrix output = new ModMatrix(row, col);
        output = output.inverse(input);

        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                outMatrix[i][j] = output.getValueAt(i, j).intValue();
            }
        }
    }

    // This class returns matrix properties with mod 26
    private static class ModMatrix {
        private int nrows;
        private int ncols;
